...
import services.xenlan.party.util.command.argument.Arguments;
import services.xenlan.party.util.command.data.Data;
import services.xenlan.party.util.command.data.FlagData;
import services.xenlan.party.util.command.data.ParameterData;
    ...
    public boolean invoke(CommandSender sender, Arguments arguments) throws CommandException {
        if (this.method == null) {
            if (this.hasCommands()) {
                ...
                sender.sendMessage(CC.chat("&fUnknown Command."));
            }
            ...
            while(true) {
                ...
                try {
                    ...
                    if (!this.async && this.logToConsole && stopwatch.getTime() >= (long)executionThreshold) {
                        xParty.getInstance().getLogger().warning("Command '/" + this.getFullLabel() + "' took " + stopwatch.getTime() + "ms!");
                    }
